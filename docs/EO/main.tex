...
\end{figure}
\subsubsection{Gerador de Interfaces para Utilizadores}

O módulo de geração de interfaces transforma os requisitos estruturados e validados em representações de elementos de interface e fluxos de interação, conforme representado na Figura \ref{fig:modulo-interfaces}. Esta etapa constitui um ponto de validação precoce, uma vez que as interfaces refletem, visualmente, a interpretação de uma parte dos requisitos. A saída deste módulo serve de referência para o frontend e também para a definição das APIs, garantindo que existe correspondência entre o que é apresentado ao utilizador e o que o sistema deve disponibilizar enquanto funcionalidade.
\begin{figure}[H]
    \centering
...
Com base nas interfaces previamente validadas e na lista estruturada de requisitos, este módulo é responsável por gerar a especificação da API, tal como a Figura \ref{fig:modulo-api} ilustra, que suporta a comunicação entre o frontend e o backend da aplicação. No contexto deste trabalho, a API é definida assumindo uma arquitetura de backend em monólito modular, servindo como contrato formal que descreve os recursos disponibilizados, os endpoints associados, as operações suportadas e os modelos de dados trocados entre as camadas do sistema. A especificação é expressa em formato OpenAPI, permitindo a geração sistemática de controladores, serviços e modelos de dados, bem como a validação do alinhamento entre as diferentes camadas da aplicação.
...
Este módulo é responsável por produzir os artefactos relacionados com a execução do sistema, como representado na Figura \ref{fig:modulo-infraestrutura}, incluindo configurações, ficheiros de suporte e componentes necessários para integrar o software em ambientes de desenvolvimento ou produção. A geração destes artefactos baseia-se na especificação OpenAPI e a lista estruturada de requisitos, a partir das quais são inferidas as características dos artefactos produzidos nas fases de geração da interface e do backend. Desta forma, é possível gerar configurações coerentes com a estrutura efetiva da aplicação, assegurando que o sistema pode ser executado em condições adequadas e alinhadas com as práticas internas da organização.
...
\subsubsection{Geração e Execução de Testes}

A geração de testes encontra-se integrada nos módulos de geração de frontend e backend da pipeline. No caso do backend, são gerados testes unitários e testes de integração básicos, diretamente derivados da especificação da API, permitindo validar o comportamento dos serviços, repositórios e endpoints implementados. No frontend, são gerados testes de componentes e testes básicos de interface, alinhados com os fluxos de interação definidos nas interfaces previamente validadas.

\noindent{Os} testes gerados constituem artefactos integrados nos componentes de frontend e backend da aplicação e são executados no ambiente preparado pela infraestrutura configurada pela pipeline, recorrendo aos mecanismos padrão de build e execução associados às tecnologias utilizadas. Desta forma, a infraestrutura fornece o suporte necessário à execução automatizada desses testes, enquanto a análise dos respetivos resultados permanece sob responsabilidade humana no âmbito da validação do sistema.
...
% \subsubsection{Avaliação dos Artefactos Gerados}
...
% \noindent{Deste} modo, a avaliação final resulta da combinação equilibrada entre métricas automáticas e avaliação humana, garantindo uma análise mais completa e representativa da qualidade global das soluções geradas.

\subsubsection{Avaliação dos Artefactos Gerados}

A avaliação dos artefactos produzidos pela pipeline proposta é realizada de forma incremental e integrada no próprio processo de desenvolvimento da solução. À medida que cada módulo da pipeline é implementado, os artefactos por si gerados são avaliados antes de se avançar para o módulo seguinte, permitindo identificar limitações, introduzir ajustes e reduzir a propagação de erros ao longo do processo.

\noindent{Esta} avaliação é conduzida tendo como referência um projeto representativo da Trust Systems, que funciona como caso de estudo para validar a adequação dos artefactos produzidos em cada etapa. Dependendo da natureza do artefacto, a avaliação recorre a métricas automáticas, validação humana ou a uma combinação de ambas, conforme descrito de seguida.\\\par

\noindent\textbf{Métricas Automáticas}

\noindent{A} avaliação automática incide sobre a qualidade do código gerado, recorrendo a métricas amplamente utilizadas na investigação em geração automática de código, nomeadamente o \textit{CodeBLEU}\cite{CODEBLEU} e o \textit{CodeBERTScore}\cite{CODEBERTSCORE}. Ambas derivam de métricas originalmente propostas para linguagem natural, tendo sido adaptadas para capturar propriedades estruturais e semânticas específicas de programas.

\paragraph{CodeBLEU}
O \textit{CodeBLEU}, proposto por Ren et al.\cite{CODEBLEU}, estende o \textit{BLEU} tradicional ao combinar múltiplas dimensões de análise, incluindo similaridade lexical, ponderação de tokens relevantes, correspondência estrutural baseada na árvore sintática abstrata (AST) e alinhamento semântico ao nível do fluxo de dados. A métrica é definida como uma combinação ponderada dos seus componentes:
...
\noindent{Na} configuração recomendada pelos autores, os pesos são definidos como \(\alpha = 0.1\), \(\beta = 0.1\), \(\gamma = 0.4\) e \(\delta = 0.4\)\cite{CODEBLEU}, atribuindo maior relevância às componentes estrutural e semântica. Os resultados experimentais demonstram que o \textit{CodeBLEU} apresenta uma correlação mais elevada com a avaliação humana do que métricas baseadas exclusivamente em sobreposição textual.

\paragraph{CodeBERTScore}
O \textit{CodeBERTScore}\cite{CODEBLEUBERTSCORE} adapta o \textit{BERTScore}\cite{BERTSCORE} ao domínio da programação, avaliando a similaridade entre o código gerado e o código de referência através de \textit{embeddings} contextuais obtidos a partir de modelos de representação de código, como o \textit{CodeBERT}\cite{CODEBERT}. A métrica define valores de \textit{precision} e \textit{recall}, combinados num valor final \textit{F1}:
...
\noindent{Ao} capturar relações semânticas para além da correspondência textual, o \textit{CodeBERTScore} revela-se particularmente adequado em cenários onde existem múltiplas implementações corretas para o mesmo problema, complementando a avaliação estrutural fornecida pelo \textit{CodeBLEU}.\\\\\\\par

\noindent\textbf{Validação Humana}

\noindent{Para} além das métricas automáticas, todos os artefactos gerados pela pipeline são sujeitos a validação humana. Esta validação permite avaliar aspetos que não são totalmente capturados por métricas quantitativas, como clareza, coerência, adequação aos requisitos e conformidade com boas práticas de desenvolvimento.

\noindent{A} validação humana desempenha igualmente um papel fundamental no processo incremental adotado, uma vez que o feedback recolhido após a avaliação de cada módulo é utilizado para refinar os artefactos produzidos antes da progressão para as etapas seguintes. Desta forma, a combinação entre métricas automáticas e validação humana assegura uma avaliação mais completa e representativa da qualidade global da solução desenvolvida.
...
\noindent{A} pipeline proposta será implementada recorrendo a uma ferramenta de orquestração de processos baseada em modelos de linguagem, cuja seleção será efetuada com base num conjunto de critérios previamente definidos. Esses critérios são a facilidade de utilização, a capacidade de integração com diferentes modelos e fornecedores de LLMs,  os custos associados e as garantias oferecidas ao nível da privacidade, segurança e tratamento de dados.

\noindent{Estas} ferramentas assumem um papel central na arquitetura da solução, funcionando como camada de controlo da pipeline e abstraindo a complexidade associada à coordenação das diferentes etapas do processo. Através desta camada, é possível estruturar fluxos de geração, gerir e versionar \textit{prompts}, encadear chamadas a modelos distintos e integrar mecanismos de validação e refinamento iterativo de forma controlada. Exemplos de plataformas que se enquadram neste tipo de soluções incluem o Dify\footnote{\url{https://dify.ai/}} e o LangFlow\footnote{\url{https://www.langflow.org/}}, que disponibilizam ambientes visuais e programáticos para a orquestração de pipelines baseadas em LLMs.

\noindent{Nesta} ferramenta de orquestração são configuradas as integrações com diferentes fornecedores de modelos de linguagem, os quais são utilizados para executar as tarefas específicas associadas a cada módulo da pipeline. Esta abordagem permite desacoplar a lógica do processo dos modelos concretos utilizados, possibilitando a seleção de LLMs distintos consoante requisitos de custo, desempenho, privacidade ou localização dos dados.

\noindent{Desta} forma, a ferramenta de orquestração não é responsável pela geração direta dos artefactos finais, mas sim pela organização, coordenação e controlo do processo que governa a utilização dos modelos de linguagem. Esta separação de responsabilidades assegura maior coerência, rastreabilidade e flexibilidade ao longo do processo de desenvolvimento.

\section{Planeamento do Trabalho} \label{sec:planeamento}

\noindent{O} processo de desenvolvimento tem início com a construção do módulo de interpretação de requisitos, seguindo-se de forma sequencial os módulos de geração de interfaces, especificação da API, backend, frontend e infraestrutura. Cada um dos módulos será avaliado e ajustado antes de ser integrado na \textit{pipeline}.\par

\noindent{A} partir do mês de abril, inicia-se em paralelo a redação do documento final, que acompanha o desenvolvimento e a avaliação da solução. Esta fase inclui a consolidação dos resultados obtidos, a descrição da metodologia adotada e a discussão dos contributos e limitações do trabalho.\par

\noindent A Figura \ref{fig:planeamento} apresenta o cronograma das atividades previstas, permitindo visualizar a sequência dos módulos da pipeline e a sobreposição temporal com a escrita do documento final.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.23\textheight, width=0.5\textwidth]{../img/planeamento.png}
