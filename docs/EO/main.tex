...
\noindent{A} integração crescente destas ferramentas nas IDEs evidencia a adoção prática dos LLMs no desenvolvimento de software, contribuindo para acelerar tarefas rotineiras e aumentar a produtividade dos programadores\cite{CopilotRole}.\\
...
\noindent{Ao} capturar relações semânticas para além da correspondência textual, o \textit{CodeBERTScore} revela-se particularmente adequado em cenários onde existem múltiplas implementações corretas para o mesmo problema, complementando a avaliação estrutural fornecida pelo \textit{CodeBLEU}.

\noindent\textbf{Validação Humana}

\noindent{Para} além das métricas automáticas, a avaliação da qualidade dos artefactos gerados integra igualmente uma componente de apreciação humana. Esta permite analisar aspetos não totalmente capturados por métricas quantitativas, como clareza, adequação ao enunciado, legibilidade e conformidade com boas práticas de programação. A combinação entre métricas automáticas e validação humana assegura uma análise mais completa e representativa da qualidade global das soluções geradas.
...
\noindent{O} trabalho a desenvolver no projeto organiza-se em três grandes etapas.\par
