...
class PersonResourceIT {
    ...
    private static final PersonStatus UPDATED_STATUS = PersonStatus.INACTIVE;
    ...
    public static Person createEntity() {
        return new Person().name(DEFAULT_NAME).email(DEFAULT_EMAIL).role(DEFAULT_ROLE).status(DEFAULT_STATUS);
    }
    ...
    public static Person createUpdatedEntity() {
        return new Person().name(UPDATED_NAME).email(UPDATED_EMAIL).role(UPDATED_ROLE).status(UPDATED_STATUS);
    }
    ...
    void putExistingPerson() throws Exception {
        ...
        updatedPerson.name(UPDATED_NAME).email(UPDATED_EMAIL).role(UPDATED_ROLE).status(UPDATED_STATUS);
    ...
    void putWithIdMismatchPerson() throws Exception {
        ...
        restPersonMockMvc
            .perform(
                put(ENTITY_API_URL_ID, UUID.randomUUID()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(personDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void fullUpdatePersonWithPatch() throws Exception {
        ...
        partialUpdatedPerson.name(UPDATED_NAME).email(UPDATED_EMAIL).role(UPDATED_ROLE).status(UPDATED_STATUS);
    ...
    void deletePerson() throws Exception {
        ...
        // Delete the person
        restPersonMockMvc
            .perform(delete(ENTITY_API_URL_ID, person.getId().toString()).accept(MediaType.APPLICATION_JSON))
