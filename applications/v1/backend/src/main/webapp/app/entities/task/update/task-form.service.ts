...
type TaskFormGroupContent = {
  ...
  text: FormControl<ITask['text']>;
  dayUserProject: FormControl<ITask['dayUserProject']>;
};
...
export class TaskFormService {
  createTaskFormGroup(task: TaskFormGroupInput = { id: null }): TaskFormGroup {
    ...
    return new FormGroup<TaskFormGroupContent>({
      ...
      text: new FormControl(taskRawValue.text, {
        validators: [Validators.required],
      }),
      dayUserProject: new FormControl(taskRawValue.dayUserProject, {
        validators: [Validators.required],
      }),
    });
  }
