...
type RestOf<T extends IWeek | NewWeek> = Omit<T, 'startDate' | 'endDate'> & {
...
export class WeekService {
  ...
  protected convertDateFromClient<T extends IWeek | NewWeek | PartialUpdateWeek>(week: T): RestOf<T> {
    return {
      ...week,
      startDate: week.startDate?.format(DATE_FORMAT) ?? null,
      endDate: week.endDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restWeek: RestWeek): IWeek {
    return {
      ...restWeek,
      startDate: restWeek.startDate ? dayjs(restWeek.startDate) : undefined,
      endDate: restWeek.endDate ? dayjs(restWeek.endDate) : undefined,
    };
  }
