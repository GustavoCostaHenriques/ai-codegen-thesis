...
import { EntityArrayResponseType, WeekService } from '../service/week.service';
...
export class WeekComponent implements OnInit {
  ...
  trackId = (item: IWeek): number => this.weekService.getWeekIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.weeks.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IWeek[] | null): IWeek[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.weekService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
